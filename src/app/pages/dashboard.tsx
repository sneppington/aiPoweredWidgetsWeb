...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const bgShapesWrapper = document.querySelector("#background-shapes-wrapper") as HTMLElement;
    // Clear any existing shapes.
    bgShapesWrapper.innerHTML = "";
    // Use the current color theme to generate initial shades.
    const initialShades = generateShadesFromRgb(colorTheme, 15);

    for (let i = 0; i < 10; i++) {
      const shape = document.createElement("div");
      // Start with a white color then transition to a theme color.
      ...
      // Randomly decide if the shape is a circle or a rotated square.
      if (randi(2) === 0) {
      ...
      // Now update the shape's background to one of the themed shades.
    ...
    const userUIClickOutDetector = (event: MouseEvent) => {
      if (pfp?.contains(event.target as Node)) return;
      pfpOptions?.style.setProperty("opacity", "0");
      pfpOptions?.style.setProperty("pointer-events", "none")
      document.removeEventListener("click", userUIClickOutDetector);
    };

    pfp?.addEventListener("click", () => {
      pfpOptions?.style.setProperty("opacity", "1");
      pfpOptions?.style.setProperty("pointer-events", "all")
    ...
    const editUIWrapper = document.querySelector("#edit-ui") as HTMLElement | null;
    const widgetWrapper = document.querySelector("#dashboard-widgets") as HTMLElement | null;

    const uploadBackgroundButton = document.querySelector("#background-image-uploader") as HTMLElement | null;
    const uploadBackgroundText = document.querySelector("#background-image-uploader-label") as HTMLElement | null;
    ...
    editButton?.addEventListener("click", () => {
      ...
      if (editing) {
        ...
        editUIWrapper?.style.removeProperty("transform");
        widgetWrapper?.style.removeProperty("width");
      }
    });
    ...
    uploadBackgroundButton?.addEventListener("change", (e) => {
      ...
      if (file) {
        ...
        reader.onload = (event) => {
          // Set the background image using the loaded data URL
    ...
    uploadBackgroundButton?.addEventListener("click", (e) => {
      if (hasBG) {
        ...
        document.querySelector(".background-filter").style.setProperty("background-image", "");
  ...
  useEffect(() => {
    // Recalculate shades and the darker variant.
    const shades = generateShadesFromRgb(colorTheme, 15);
    const darkerShade = darkenColor(colorTheme);

    // Set a CSS variable for the theme.
    document.documentElement.style.setProperty("--colorTheme", colorTheme);

    // Update the header's background with a gradient that uses the darker shade.
    const landingHeader = document.querySelector("#page-header") as HTMLElement;
    landingHeader.style.background = `linear-gradient(to top, ${darkerShade}, rgba(0,0,0,0) 100%)`;

    // Update colors of all background shapes.
    const bgShapesWrapper = document.querySelector("#background-shapes-wrapper") as HTMLElement;
